...
parser.add_argument('--audio_quality', type=str, default='high', choices=['low', 'medium', 'high'], help='Audio quality')
...
def transcribe_file(filename):
    ...
    try:
        ...
        # Real-time transcription progress (streaming mode)
        # Uncomment below lines to enable streaming mode transcription
        # for chunk in model.transcribe(processed_path, stream=True):
        ...
        if confidence < 0.7:
            print(f"Low confidence transcription for {filename} with score {confidence}")
        ...
        # Optional: Upload to S3 (uncomment and set bucket_name)
...
try:
    with ThreadPoolExecutor() as executor:
        list(tqdm.tqdm(executor.map(transcribe_file, audio_files), total=len(audio_files), desc="Transcribing"))
